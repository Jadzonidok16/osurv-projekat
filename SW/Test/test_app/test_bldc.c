...
int16_t speed_to_duty(int speed) {
	printf("speed = %d\n", speed);
...
int main(int argc, char** argv){
	...
	motor_ctrl__read_arg_fb_t ba;
	r = read(fd, (char*)&ba, sizeof(ba));
	if(r != sizeof(ba)){
		fprintf(stderr, "ERROR: read went wrong!\n");
		return 4;
	} 
	
	int64_t new_pulse_cnt_fb = ba.pulse_cnt_fb[0]; 
	printf("new pulse_cnt_fb = %lld\n", (long long int)new_pulse_cnt_fb);
	
	duty = speed_to_duty(0); // Stop motor.
